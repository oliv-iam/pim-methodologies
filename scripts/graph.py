...
def neat(df):
    ...
    for col in to_return.columns:
        if col == 'Name':
            to_return['Name'] = to_return['Name'].str.strip('.').astype(float) # numerical values
            # to_return['Name'] = to_return['Name'].replace('dram_type_', '', regex=True).replace('.cfg.txt', '', regex=True) # strings
            continue
